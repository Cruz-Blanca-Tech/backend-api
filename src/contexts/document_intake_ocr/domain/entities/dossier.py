...
class Dossier:
    def __init__(self, dni: str, activity_id: uuid.UUID, batch_id: uuid.UUID):
        self.dni = dni
    ...
    def is_complete(self, requirements: List[ActivityRequirement]) -> bool:
        present_codes = {doc.document_type_code for doc in self.documents if doc.document_type_code}
    ...
    def update_status(self, requirements: List[ActivityRequirement]) -> None:
        missing = [req.document_config.code for req in requirements if req.is_required and req.document_config.code not in {d.document_type_code for d in self.documents}]
        if not missing:
