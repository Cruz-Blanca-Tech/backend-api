...
class ReligionEnrichedMapper(BaseEnrichedMapper):
    def map(self, raw_dto: FinsRaw) -> EnrichedReligion:
        return EnrichedReligion(
            baptized=self.build_field(raw_dto.religion_baptized, "¿Fue bautizado?", DataType.BOOL),
            first_communion=self.build_field(raw_dto.religion_first_communion, "¿Hizo la 1ra Comunión?", DataType.BOOL)
        )
